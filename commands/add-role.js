const {smartRoleResolver,determineMentionType,getUsersFromRole,getUserIDFromMention,resolveMemberFromID,embedify} = require('../common')
...
module.exports = {
    ...
    async execute(msg, args){
        let member = await resolveMemberFromID(msg.author.id,msg.guild)
        ...
        let addRoleRes = await smartRoleResolver(addRole,msg.guild)
        ...
        for (let arg of args) {
            ...
            switch (typedArg.type) {
                case "role":
                    let targetRoleID = await smartRoleResolver(typedArg.mention).id
